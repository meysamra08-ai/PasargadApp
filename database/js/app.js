...
async function clearActiveFiscalYear() {
    ...
    try {
        ...
        await loadCheques();
        await fiscalDialog({title:"عملیات موفق", message:toPersianDigits(result?.changes || 0) + " چک از سال مالی " + toPersianDigits(year) + " حذف شد.", okText:"باشه", cancelText:""});
    } catch (error) {
        console.error(error);
...
async function previewReportFromModal(){
    ...
    try {
        ...
        const reportClone = buildPrintableReportClone();
...
async function loadAppearanceSettings(){
    try{
        ...
        const company=document.getElementById("appearanceCompanyName"); const signer1=document.getElementById("appearanceSigner1"); const signer2=document.getElementById("appearanceSigner2"); const logo=document.getElementById("appearanceLogoPath");
