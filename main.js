...
function splashImagePath() {
    return path.join(__dirname, "assets", "splash.png");
}
function showSplash() {
    ...
    splashWindow.loadFile("splash.html").then(() => splashWindow.show());
    splashWindow.on("closed", () => { splashWindow = null; });
}
async function closeSplashWhenReady() {
    const wait = Math.max(0, 5000 - (Date.now() - splashStartTime));
    setTimeout(() => {
...
ipcMain.handle("get-splash-image", async () => ({ success: true, path: fileUrl(splashImagePath()) }));
...
function createWindow() {
    ...
    win.once("ready-to-show", () => closeSplashWhenReady());
...
/* =========================
   مدیریت مشتریان
   تمام IPCهای این بخش فقط برای ماژول مدیریت مشتریان هستند.
========================= */
ipcMain.handle("get-customers", async (event, data) => { try { return await database.getCustomers(data || {}); } catch(e){ return {success:false,error:e.message}; } });
ipcMain.handle("add-customer", async (event,data) => { try { const r=await database.addCustomer(data||{}); if(r&&r.success) scheduleAutoBackup(); return r; } catch(e){ return {success:false,error:e.message}; } });
ipcMain.handle("update-customer", async (event,data) => { try { const r=await database.updateCustomer(data||{}); if(r&&r.success) scheduleAutoBackup(); return r; } catch(e){ return {success:false,error:e.message}; } });
ipcMain.handle("delete-customer", async (event,data) => { try { const r=await database.deleteCustomer(data || {}); if(r&&r.success) scheduleAutoBackup(); return r; } catch(e){ return {success:false,error:e.message}; } });
ipcMain.handle("get-customer-profile", async (event,data) => { try { return await database.getCustomerProfile(data || {}); } catch(e){ return {success:false,error:e.message}; } });
ipcMain.handle("get-customer-follow-ups", async (event,data) => { try { return await database.getCustomerFollowUps(data || {}); } catch(e){ return {success:false,error:e.message}; } });
ipcMain.handle("add-customer-contact", async (event,data) => { try { const r=await database.addCustomerContact(data||{}); if(r&&r.success) scheduleAutoBackup(); return r; } catch(e){ return {success:false,error:e.message}; } });
ipcMain.handle("update-customer-follow-up", async (event,data) => { try { const r=await database.updateCustomerFollowUp(data||{}); if(r&&r.success) scheduleAutoBackup(); return r; } catch(e){ return {success:false,error:e.message}; } });

/* =========================
   کنترل ارسال - فقط ماژول کنترل ارسال
========================= */
ipcMain.handle("get-send-control-records", async (event,data) => { try { return await database.getSendControlRecords(data||{}); } catch(e){ return {success:false,error:e.message}; } });
ipcMain.handle("add-send-control-record", async (event,data) => { try { const r=await database.addSendControlRecord(data||{}); if(r&&r.success) scheduleAutoBackup(); return r; } catch(e){ return {success:false,error:e.message}; } });
ipcMain.handle("update-send-control-customer-name", async (event,data) => { try { const r=await database.updateSendControlCustomerName(data||{}); if(r&&r.success) scheduleAutoBackup(); return r; } catch(e){ return {success:false,error:e.message}; } });
ipcMain.handle("update-send-control-status", async (event,data) => { try { const r=await database.updateSendControlStatus(data||{}); if(r&&r.success) scheduleAutoBackup(); return r; } catch(e){ return {success:false,error:e.message}; } });
